...
const ApplicationStatus = () => {
  ...
  const handleSubmitApplication = async () => {
    if (
      !window.confirm(
        "Apakah Anda yakin ingin submit aplikasi ini? Setelah submit, Anda tidak dapat mengedit sampai admin memberikan feedback."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/pmi/my-application/submit",
        {},
        config
      );
    ...
    <div>
                  ...
                  >
                    <div
                      style={{
                        ...
                        boxShadow:
                          "0 0 0 2px " + getStatusColor(history.status),
                      }}
                    />
      ...
      <div className="dashboard-grid">
        <div className="dashboard-card">
          ...
          <div style={{ fontSize: "0.9em" }}>
            ...
            <p>
              <strong>Asal:</strong> {application.asal.kecamatan},{" "}
              {application.asal.desa}
            </p>
            <p>
              <strong>Jenis Kelamin:</strong> {application.jenisKelamin}
            </p>
            <p>
              <strong>Pendidikan:</strong> {application.pendidikanTerakhir}
            </p>
          </div>
        </div>
        ...
        <div className="dashboard-card">
          ...
          <div style={{ fontSize: "0.9em" }}>
            <p>
              <strong>Negara Tujuan:</strong> {application.negaraTujuan}
            </p>
            <p>
              <strong>Profesi:</strong> {application.profesi}
            </p>
            <p>
              <strong>Rencana Berangkat:</strong>{" "}
              {new Date(application.waktuBerangkat).toLocaleDateString("id-ID")}
            </p>
            {application.keterampilan &&
              application.keterampilan.length > 0 && (
                <p>
                  <strong>Keterampilan:</strong>{" "}
                  {application.keterampilan.join(", ")}
                </p>
              )}
          </div>
        </div>
      ...
      <div className="dashboard-card" style={{ marginTop: "20px" }}>
        ...
        <div style={{ marginTop: "15px" }}>
          {Object.entries(application.dokumen).filter(([key, path]) => path)
            .length > 0 ? (
            <div className="table-container">
              <table className="data-table">
                ...
                <tbody>
                  {Object.entries(application.dokumen)
      ...
      >
        ...
        {application.status === "draft" && (
          <>
            ...
            <button
              className="edit-btn"
              onClick={() => navigate("/user/application/edit")}
            >
              Edit Aplikasi
            </button>
