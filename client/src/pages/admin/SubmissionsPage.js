...
const SubmissionsPage = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/pmi/submissions', config);
  ...
  const handleSubmitAction = async () => {
    if (actionType === 'approve' || actionType === 'reject' || actionType === 'revision') {
      if (!actionNote.trim() && (actionType === 'reject' || actionType === 'revision')) {
            ...
            <div key={submission._id} className="dashboard-card">
              ...
              <div style={{ fontSize: '0.9em', marginBottom: '15px' }}>
                <p style={{ margin: '5px 0' }}>
                  <strong>Asal:</strong> {submission.asal.kecamatan}, {submission.asal.desa}
                </p>
                <p style={{ margin: '5px 0' }}>
                ...
                <p style={{ margin: '5px 0' }}>
                  <strong>Disubmit:</strong> {submission.submittedAt ? new Date(submission.submittedAt).toLocaleDateString('id-ID') : '-'}
                </p>
              </div>
          ...
          <div className="modal-content" style={{ maxWidth: '800px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginTop: '20px' }}>
              <div className="form-section">
                <h4>Data Diri</h4>
                <p><strong>ID PMI:</strong> {selectedSubmission.pmiId}</p>
                <p><strong>Nama:</strong> {selectedSubmission.nama}</p>
                <p><strong>Asal:</strong> {selectedSubmission.asal.kecamatan}, {selectedSubmission.asal.desa}</p>
                <p><strong>Jenis Kelamin:</strong> {selectedSubmission.jenisKelamin}</p>
                <p><strong>Pendidikan:</strong> {selectedSubmission.pendidikanTerakhir}</p>
                {selectedSubmission.keterampilan && selectedSubmission.keterampilan.length > 0 && (
                  <p><strong>Keterampilan:</strong> {selectedSubmission.keterampilan.join(', ')}</p>
                )}
              </div>

              <div className="form-section">
                <h4>Data Keberangkatan</h4>
                <p><strong>Negara Tujuan:</strong> {selectedSubmission.negaraTujuan}</p>
                <p><strong>Profesi:</strong> {selectedSubmission.profesi}</p>
                <p><strong>Rencana Berangkat:</strong> {new Date(selectedSubmission.waktuBerangkat).toLocaleDateString('id-ID')}</p>
              </div>
              ...
              <div className="form-section">
                ...
                <div className="table-container">
                  <table className="data-table">
                    ...
                    <tbody>
                      {Object.entries(selectedSubmission.dokumen)
                        .map(([key, path]) => (
                          <tr key={key}>
                            <td>{key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</td>
                            <td>
                              {path ? (
                                <span style={{ color: 'var(--success-color)', fontWeight: 'bold' }}>✓ Ada</span>
                              ) : (
                                <span style={{ color: 'var(--error-color)' }}>✗ Belum</span>
                              )}
                            </td>
                            <td>
                              {path && (
                                <>
                                  ...
                                  <button 
                                    ...
                                    onClick={() => handleDownloadDocument(selectedSubmission._id, key, 'document')}
                                  >
                                    Unduh
                                  </button>
                                </>
      ...
      {showActionModal && selectedSubmission && (
        <div className="modal">
          <div className="modal-content" style={{ maxWidth: '500px' }}>
            <button className="close-btn" onClick={() => setShowActionModal(false)}>&times;</button>
            ...
            <p style={{ marginBottom: '15px', color: 'var(--text-secondary)' }}>
              Aplikasi: <strong>{selectedSubmission.nama}</strong> ({selectedSubmission.pmiId})
            </p>
            ...
            <div className="form-buttons">
              <button 
                className={
                  actionType === 'approve' ? 'submit-btn' :
                  actionType === 'revision' ? 'edit-btn' : 'delete-btn'
                }
